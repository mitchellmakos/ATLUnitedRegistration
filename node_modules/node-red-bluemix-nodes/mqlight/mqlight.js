...
module.exports = function(RED) {
    ...
    var mlservices = appEnv.services.mqlight || [];
    var mhservices = appEnv.services.messagehub || [];
    var mlserviceList = mlservices.map(function(s) { return s.name; });
    var mhserviceList = mhservices.map(function(s) { return s.name; });
    var services = mlservices.concat(mhservices);
            ...
            var recvClient = mqlight.createClient(opts, function(err) {
                if (err) {
                    ...
                    var subscribeCallback = function(err) {
                        if (err) { node.error("Failed to subscribe: " + err); }
                        else { node.log("Subscribed to "+node.topic+(node.share ? " ["+node.share+"]" : "")); }
                    };

                    if (node.share) { recvClient.subscribe(node.topic, node.share,subscribeCallback); }
                    else { recvClient.subscribe(node.topic,subscribeCallback); }
                }
            });
